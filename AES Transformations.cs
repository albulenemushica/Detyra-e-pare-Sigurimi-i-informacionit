...
namespace AES_Operations
{
    class AES_Transformations
    {
        static byte[] s_box = new byte[256] {
        ...
        static void KeyExpansionCore(byte[] inR, int i)
        {
            //rotate left
            byte t = inR[0];
            inR[0] = inR[1];
        ...
        static void KeyExpansion(byte[] inputKey, byte[] expandedKeys)
        {
            ...
            int bytesGenereated = 16;
            int rconIteration = 1;
            var temp = new byte[4];

            while (bytesGenereated < 176)
            {
                for (int i = 0; i < 4; i++)
                    temp[i] = expandedKeys[i + bytesGenereated - 4];//4 last bytes

                if (bytesGenereated % 16 == 0)
                    KeyExpansionCore(temp, rconIteration++);
                //XOR temp with bytesGenereated - 16 and store in expandedKeys
                for (int a = 0; a < 4; a++)
                {
                    expandedKeys[bytesGenereated] = (byte)(expandedKeys[bytesGenereated - 16] ^ temp[a]);
                    bytesGenereated++;
                }
        ...
        static void AES_Encrypt(byte[] message, byte[] key)
        {
            ...
            for (int i = 0; i < numberOfRounds; i++)
            {
                ...
                ShiftRows(state);
                MixColumns(state); 
                byte[] array = expandedKey;// 32          16*3=48
                //byte[] newArray = array.Skip(i*16).Take(16 * (i + 1)).ToArray();
                byte[] newArray = array.Skip(i*4).Take(4* (i + 1)-1).ToArray();
                AddRoundKey(state, newArray);
            }
            //final round
            SubBytes(state);
            ShiftRows(state);
            byte[] array1 = expandedKey;
            //AddRoundKey(state, expandedKey + 160);
            byte[] newArray1 = array1.Skip(160).Take(16).ToArray();
            AddRoundKey(state, newArray1);
            //copy over the message with the encrypter message
        ...
        {
            //char message = Convert.ToChar("This is a message we will encrypt with AES!");

            byte[] message = Encoding.ASCII.GetBytes("Encrypt message!");
            ...
            byte[] key =  {1, 2, 3, 4,
                            ...
                            9, 10, 11, 12,
                            13, 14, 15, 16};
            //length of message, null as string 
              ...
              var paddedMessage = new byte[lenOfPaddedMessage];
            ...
            //Print Message
            Console.Write("Plaintext: Encrypt message! \n");
            Console.Write("Message: ");
            for (int i = 0; i < 16; i++)
                Console.Write("{0:X} ", paddedMessage[i]);
            Console.Write("\n");


            //Print SubBytes State
            SubBytes(paddedMessage);
                Console.Write("SubByte State: ");
            for (int i = 0; i < 16; i++)
                Console.Write("{0:X} ", paddedMessage[i]);
            Console.Write("\n");

            //Print ShiftRows State
            ShiftRows(paddedMessage);
            Console.Write("ShiftRows State: ");
            for (int i = 0; i < 16; i++)
                Console.Write("{0:X} ", paddedMessage[i]);
            Console.Write("\n");
            ...
            MixColumns(paddedMessage);
            Console.Write("MixColumns State: ");
            for (int i = 0; i < 16; i++)
                Console.Write("{0:X} ", paddedMessage[i]);
            Console.Write("\n");

            //Print AddRoundKey State
            AddRoundKey(paddedMessage, key);
            Console.Write("AddRoundKey State / Initial Round: ");
            for (int i = 0; i < 16; i++)
                Console.Write("{0:X} ", paddedMessage[i]);
            Console.Write("\n");
